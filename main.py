...
def run_sql_query(user_query, user_id, conversation_history=None):
    ...
    messages = [{"role": "system", "content": f"""You are an AI assistant for a university registrar system.
            ...
            - Do not guess or expose data for other users. Only produce SQL SELECT queries. 
            - When asked a question please generate and return more results than necessary. i.e. when asked for a advisor return their first and last names with an email etc.
            - IMPORTANT: if someone asks what classes they still have to take to graduate or what classes they should take next semester or anything related it is equivalent to asking what classes they have left in their form.. (please lookup and return any classes they have not taken yet in their form)
...
def run_rag_query(user_query, conversation_history=None):
    if conversation_history is None:
        conversation_history = []
    # Azure OpenAI client
...
def revised(question, sql_json=None, rag_text=None, conversation_history=None):
    if conversation_history is None:
        conversation_history = []
...
def chat():
    ...
    if user["user_type"] == 2:
        ...
        (for example: courses offered/what classes are taught, time of courses, student grades, student records, finding a students advisor, looking up a current student schedule, what classes someone should take etc.)
        Specific case: if a user asks what classes they should take next semester, cut out the next semester part from the question.

        You should return RAG if the question is asking about generic questions 
        (for example: graduation requirements, academic policies, available programs, etc.)
